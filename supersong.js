...
*/
	...
	this.play_flows = function(s){
		...
		this.play(0, 0, 0.0001, segment_id, this.reactToSegment, 0)
	...
	this.play_list = function(play_segment_list){
		...
		if(this.sound_loaded){
			...
			for(var loops=0;loops<32;loops++){
				for(var s=0;s<play_segment_list.length;s++){
					segment_id = play_segment_list[s].segment_id
					segment = this.segments[segment_id]
...
function Loop (song, tempo, steps, pulses, playhead_selections, palette_selections, play_segment_list, chaos, flows ,forever){
	this.song = song
	this.tempo = tempo
	this.steps = steps
	this.pulses = pulses
