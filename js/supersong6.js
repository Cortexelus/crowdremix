	...
	function Playhead(playhead_id, playhead_highlight_id, segments, palette){
		...
		this.highlight = function(highlightSegmentIDs){
			...
			for (h=0; h<highlightSegmentIDs.length; h+=1){ 
				...
				hue = this.palette_hues[this.palette_segments.indexOf(parseInt(s,10))]
				//console.log(hue)
...
*/
	...
	function RhythmDisplay(rhythm_display_id){
		var step_length, percent_length, pixel_length, segment_id, hue, c1, c2, rd, beat_bars;
		...
		this.draw = function(steps,play_segment_list){
			...
			for(var s=0;s<play_segment_list.length;s++){
				...
				hue = _song.palette_hues[_song.palette_segments.indexOf(parseInt(segment_id,10))]
				//console.log("rhythm_display",play_segment_list, segment_id,_song.palette_segments.indexOf(parseInt(segment_id,10)), _song.palette_hues, hue)
    ...
    this.rhythmChange = function() {
      ...
      $('#GC').attr("style","width: "+(100 * (_song.steps-1)/16)+"%");
    ...
    this.load_loop = function(loop){
		...
		_song.chaos = loop.chaos || "order";
		_song.flows = loop.flows || "flows";
		_song.forever = loop.forever || "forever";
		_song.layer = loop.layer || "on earth"
    ...
    this.save_loop = function(){
    	...
    	loop = new Loop(song = this, 
    		...
    		playhead_selections = _song.playhead_selections.slice(),
    		palette_selections = _song.palette_selections.slice(),
    		play_segment_list = _song.play_segment_list.slice(),
    		chaos = _song.chaos.slice(),
    		flows = _song.flows.slice(),
    		forever = _song.forever.slice(),
    		layer = _song.layer.slice());
	...
	this.play2 = function(seg){
		//console.log("play2",seg.when, seg.segment_id)
		this.sound.queueSegment(seg);
	}
	this.queueBar = function(barFun, when){
		//console.log("play2",seg.when, seg.segment_id)
		this.sound.queueBar(barFun, when);
	}
		...
		if(this.sound_loaded){
			...
			for(var loops=0;loops<num_loops;loops++){
				for(var s=0;s<play_segment_list.length;s++){
					...
					/*this.play2({
						when: then - this.latency,
						start: 0,
						duration: 0.001,
						segment_id: segment_id,
						gain: 0,
						layer: this.layer,
						callback_onended: this.reactToSegment
					})*/
			...
			if(this.forever == "and so on"){
				...
				this.play2({
					when: then,
					start: segment.start + segment_duration,
					duration: 999999,
					segment_id: 0,
					gain: 1,
					layer: this.layer,
...
function Loop (song, tempo, steps, pulses, playhead_selections, palette_selections, play_segment_list, chaos, flows ,forever, layer){
    ...
    this.draw_key = function(song, key_id){
		...
		for(var s=0;s<this.play_segment_list.length;s++){
			...
			hue = song.palette_hues[song.palette_segments.indexOf(parseInt(segment_id,10))]
