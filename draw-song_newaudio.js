		...
		this.highlight = function(highlightSegmentIDs){
			...
			for (h=0; h<highlightSegmentIDs.length; h+=1){ 
				...
				if(false){
					...
					context.rect(x, new_height/2, 1, (height - new_height)/2);
				}else{
		...
		this.draw = function (){
			...
			for (s=0; s<totalSegments; s+=1){ 
				...
				if(segment.hasOwnProperty("loudness_max")){
					// change height according to loudness
					new_height = height * (60-segment.loudness_max)/60
					console.log(new_height)
					context.rect(x, 0, 1, (height - new_height)/2);
				}else{
					context.rect(x, 0, 1, height);
				}
...
*/
    ...
    this.load_loop = function(loop){
		...
		this.play_list(loop.play_segment_list)
	...
	sound.init();
	this.sound = sound
	this.play = function(w,s,d, callback, gain){
		this.sound.play(w,s,d, callback,gain)
	};
		...
		if(this.sound_loaded){
			...
			for(var loops=0;loops<10;loops++){
				for(var s=0;s<play_segment_list.length;s++){
					...
					this.play(then, 0, 0.0001,segment_id,this.reactToSegment,0)
					if(true){//step_duration <= segment.duration){
						...
						this.play(then, segment.start, segment.duration, segment_id, this.pauseVideo, 1 )
					}
...
function Loop (song, tempo, steps, pulses, playhead_selections, palette_selections, play_segment_list){
	this.song = song
	this.tempo = tempo
	this.steps = steps
	this.pulses = pulses
	...
	this.get_samples = function(callback){
		...
		console.log(song.sound.source.buffer.numberOfChannels,total_duration,song.sound.source.buffer.sampleRate)
		var offline = new webkitOfflineAudioContext(song.sound.source.buffer.numberOfChannels,
			total_duration*song.sound.source.buffer.sampleRate,
