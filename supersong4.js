...
*/
    ...
    var step_slider = $('#step_slider').slider()
            ...
            .on('mouseup', function(){ _song.play_list(_song.play_segment_list) })
            .data('slider');
		...
		if(this.sound_loaded){
			...
			for(var loops=0;loops<num_loops;loops++){
				for(var s=0;s<play_segment_list.length;s++){
					...
					// create event to trigger when sample starts
					this.play(then, 0, 0.0001,segment_id,this.reactToSegment,0, this.layer)
	...
	this.stop_audio = function(layer){
		...
		try{
			if(layer==undefined){
				...
				this.sound.stopLayer(0,"on earth")
				this.sound.stopLayer(0,"on neptune")
			}
		}catch(e){
